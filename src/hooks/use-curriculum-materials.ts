...
import { CurriculumMaterial, CurriculumPhase, PhaseTask } from '@/lib/types/database'

export function useCurriculumMaterials(profileId: string | undefined, curriculumYear?: string) {
  ...
  const fetchAll = useCallback(async () => {
    if (!profileId) {
      setMaterials([]); setPhases([]); setPhaseTasks([])
      setLoading(false)
      return
    }
    try {
  ...
  // Phase Tasks CRUD
  const addTask = async (task: { phase_id: string; task_name: string; order_index?: number }) => {
    const existingTasks = phaseTasks.filter(t => t.phase_id === task.phase_id)
    const { error: err } = await supabase
      .from('phase_tasks')
      .insert({ ...task, order_index: task.order_index ?? existingTasks.length })
    if (err) throw err
    await fetchAll()
  }
  ...
  const deleteTask = async (id: string) => {
    const { error: err } = await supabase.from('phase_tasks').delete().eq('id', id)
    if (err) throw err
    await fetchAll()
  }
