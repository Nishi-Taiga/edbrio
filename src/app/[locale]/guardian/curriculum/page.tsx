...
export default function GuardianCurriculumPage() {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- user?.id already covers user dependency
      ...
      try {
        const [matsRes, examsRes, scoresRes] = await Promise.all([
          ...
          supabase
            .from("exam_schedules")
            .select("*")
            .eq("student_id", selectedProfileId)
            .order("exam_date"),
          supabase
            .from("test_scores")
            .select("*")
  ...
  const subjectProgress = (() => {
    ...
    materials.forEach((m) => {
      if (!subjects.has(m.subject))
        subjects.set(m.subject, { total: 0, completed: 0 });
    });
    phases.forEach((p) => {
                  ...
                  <Card>
                    <CardContent className="pt-6">
                      <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm">
                        ...
                        {selectedProfile.curriculum_year && (
                          <div>
                            <span className="text-muted-foreground">
                              {t("year")}:
                            </span>{" "}
                            ...
                            {latestScores.map((score) => {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <div className="flex items-center gap-2">
                                    <span
                                      className={`font-mono ${pct >= 80 ? "text-green-600" : pct >= 60 ? "" : "text-red-600"}`}
                                    >
