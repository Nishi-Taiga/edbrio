...
export function TestScoreForm({
  ...
  useEffect(() => {
    if (open) {
      ...
      setMaxScore(
        initialData?.max_score != null ? String(initialData.max_score) : "100",
      );
      setPercentile(
        initialData?.percentile != null ? String(initialData.percentile) : "",
      );
      setIsMainSubject(initialData?.is_main_subject ?? true);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {isEdit ? t("editTestScore") : t("addTestScore")}
          </DialogTitle>
          <DialogDescription>
            {isEdit
              ? t("editTestScoreDescription")
              : t("addTestScoreDescription")}
          </DialogDescription>
        </DialogHeader>
